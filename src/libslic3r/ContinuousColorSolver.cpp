// Vendored 2026-08-21 from ratdoux/OrcaSlicer-FullSpectrum (AGPL-3.0-or-later) @ a7c584bd9d97b202983d74a8905b955ecca1a906,
// the FullSpectrum colour->filament solver (Kubelka-Munk / ICC), brought into PrintMan to classify a
// shader's Cout to filament weights per face. Modified: the mixed-filament-manager blend fallback (malformed-colour path only) is stubbed to the first component's colour, so FS's stateful mixed-filament manager is not vendored.
    ...
    std::function<void(size_t, int)> enumerate = [&](size_t component_index, int remaining_units) {
        if (component_index + 1 == units.size()) {
            ...
            if (ks_inputs.size() == 1) {
                ...
                // Retain a usable candidate table for malformed legacy colors. Valid physical colors always
                // take the KM/K-S path above; vendored into PrintMan, the mixed-filament-manager blend
                // fallback is replaced by the first component's colour (this path is unreachable for valid input).
                mixed_hex = ks_inputs.front().color_hex;
            }
